...
$homegroupsContent = require __DIR__ . '/content/homegroups.php';
$page = Page::getByPath('/community/homegroups', 'ACTIVE');
...
if (!$page instanceof Page || $page->isError()) {
    ...
    $page = $parent->add(
        $pageType,
        [
            'cName' => $homegroupsContent['name'],
            'cHandle' => 'homegroups',
            'cDescription' => $homegroupsContent['description'],
        ],
        $fullTemplate
    );

    $output->writeln('<info>Created /community/homegroups.</info>');
}
...
$page->addBlock($contentBlockType, $area, ['content' => $homegroupsContent['content']]);

$output->writeln('<info>Updated Homegroups page content from the leadership questionnaire.</info>');
